...
export default function Sidebar({ mobileOpen, onClose }: Props) {
  ...
  return (
    <>
      ...
      <aside className={`
        fixed top-0 left-0 z-50 h-full w-64 bg-sidebar text-white flex flex-col
        ...
        <div className="p-4 border-t border-white/10">
          <div className="flex items-center gap-3 mb-3">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
              role === 'owner' ? 'bg-amber-500/30 text-amber-300' : 'bg-primary/30 text-primary-light'
            }`}>
              {user.charAt(0).toUpperCase()}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate">{user}</p>
              <p className={`text-xs capitalize ${role === 'owner' ? 'text-amber-400' : 'text-slate-400'}`}>
